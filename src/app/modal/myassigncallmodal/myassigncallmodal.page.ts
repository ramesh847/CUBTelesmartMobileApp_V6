...
console.log(addressname)
  ...
  changeCallOutcome(event) {
    ...
    console.log(event);
    if (event.target.value == 'Calls-FollowUp') {
  ...
  onSubmitForm (MyassignCallForm) {
    ...
    var responseid =this.MyassignCallForm.get('mobile').value;
    ...
    // if (purpose == "5" && $scope.assigned.current == "Y") {
    //   if (($scope.assigned.collected_date == null || $scope.assigned.collected_date == undefined || $scope.assigned.collected_date == "") || ($scope.assigned.collected_accno == null || $scope.assigned.collected_accno == "" || $scope.assigned.collected_accno == undefined) || ($scope.assigned.collectamount == null || $scope.assigned.collectamount == "" || $scope.assigned.collectamount == undefined)) {
    ...
    //     $scope.Collectiondate1 = $scope.assigned.collected_date;
    //     var collectiondate = $filter('date')($scope.Collectiondate1, 'yyyy-MM-dd');
    //     var accountno = $scope.assigned.collected_accno;
    //     var amount = $scope.assigned.collectamount;
    //   }
...
console.log(branchid, cbsid, customername, mobile, CallerId, username, calltype, remarks1, purpose, responseid, nextcalldate, firstname, lastname, usercode, callid, accountno, amount, collectiondate, collectionmode, jointvisit, jointcode, Endtime, Totime, BusinessUnit)
if (this.calltype == "P") {
  // $scope.showspin();
  this.Apiservice.updateassignedcalls(branchid, cbsid, customername, mobile, CallerId, username, calltype, remarks1, purpose, responseid, nextcalldate, firstname, lastname, usercode, callid, accountno, amount, collectiondate, collectionmode, jointvisit, jointcode, Endtime, Totime, BusinessUnit).then(response =>{
